defmodule Philosopher do
  ...
  def make_order(waiter, pos, backoff) do
  ...
  def dream(hunger, right, left, name, ctrl, timeout, waiter, pos, backoff) do
    ...
    case make_order(waiter, pos, timeout) do # with waiter
      :ok -> # with waiter
        case Chopstick.request(left, right, timeout + backoff) do
          :ok ->
      ...
      :no ->
        send(ctrl, :timeout)
        dream(hunger - 1, right, left, name, ctrl, timeout, waiter, pos, backoff + 1600) # with waiter
    end # with waiter
